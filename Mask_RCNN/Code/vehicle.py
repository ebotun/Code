...
class VehicleConfig(Config):
    ...
    STEPS_PER_EPOCH = 100
    DETECTION_MIN_CONFIDENCE = 0.99 #how confident should the AI be to color the cars in the picture? 0.99 = 99% confident
    IMAGE_MAX_DIM = 1024
    IMAGE_MIN_DIM = 256
class VehicleDataset(utils.Dataset):
    ...
    def load_vehicle(self, dataset_dir, subset):
        ...
        for a in annotations:
            ...
            self.add_image("vehicle",image_id=a['filename'],path=image_path,width=width, height=height,polygons=polygons)
...
def color_splash(image, mask):
    color = [100,0,0] #select color on vehicle found in image RGB
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', required=False,
                        default=DEFAULT_DATASET_FOLDER+"vehicle", #can change default to another folder, or provide path with --dataset
